...
class ChestXRayModelTrainer:
    def __init__(self, model_params, client_config, client_url):
    ...
    def train_model(self):
        ...
        # Save model to disk (hay que cambiarlo a: hashear el modelo)
        model_json = model.to_json()
        split_url = str(self.client_url[:-1]).split(':')
        with open("model_"+str(split_url)+".json", "w") as json_file:
            json_file.write(model_json)
        model.save_weights("weights"+str(split_url)+".h5")
...
chestXRayModelTrainer = ChestXRayModelTrainer(None, None, None)
